...
export async function ensureS3BucketConfig(prompting: boolean = true) {
  ...
  if (!result.success) {
    if (prompting) {
      ...
      const s3BucketRegion = await input({ message: 'what is your s3 bucket region?' });
      ...
      throw new Error(
        `prompting is disabled so $S3_BUCKET_ENDPOINT, $S3_BUCKET_PORT, $S3_BUCKET_REGION, $S3_BUCKET_ACCESS_KEY and $S3_BUCKET_SECRET_KEY environment variables must be provided`
      );
    }
  } else {
    ...
    config.region = result.data.S3_BUCKET_REGION;
